...
public sealed class TypeRefFactory(CompiledContracts contracts)
{
    ...
    private TypeRef.Types.Known? TryKnownTypeRef(ITypeSymbol ts)
    {
        return ts switch
        {
            ...
            { ContainingNamespace: { Name: "Contracts", ContainingNamespace.Name: "LeanCode" }, Name: "Binary" } => New(
                KnownType.Binary
            ),
            {
            ...
            _ when contracts.Types.IsQueryType(ts) => New(
                KnownType.Query,
                From(contracts.Types.ExtractQueryResult(ts))
            ),
            _ when ts is INamedTypeSymbol ns && contracts.Types.IsCommandType(ns) => New(KnownType.Command),
            _ when contracts.Types.IsOperationType(ts) => New(
                KnownType.Operation,
                From(contracts.Types.ExtractOperationResult(ts))
            ),
            ...
            _ when contracts.Types.IsAuthorizeWhenHasAnyOfType(ts) => New(KnownType.AuthorizeWhenHasAnyOfAttribute),
            ...
            _
                when ts is INamedTypeSymbol ns
                    && ns.Arity == 1
                    && ns.Interfaces.Any(i => i.SpecialType == SpecialType.System_Collections_IEnumerable) => New(
                KnownType.Array,
                From(ns.TypeArguments[0])
            ),
